"""One-time migration: add backdrop_path to existing movie documents."""
...
async def main():
    ...
    # Find movies without backdrop_path
    ...
    total = len(movies)
    logger.info(f"Found {total} movies without backdrop_path")

    if total == 0:
        logger.info("All movies already have backdrop_path. Nothing to do.")
        client.close()
        return
